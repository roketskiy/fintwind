...
- 假定 `bun ./scripts/dev.ts` 已经在运行,并且持有当前的 `Fintwind Debug.app`
  进程。源码变更会被自动重新编译、签名并重启。只有在它尚未启动时才需要你自己运行。
- 每次修改之后,直接编译release，如有特殊要求，不然不打包zip和安装包。
- 除非被要求,否则不做视觉测试。
