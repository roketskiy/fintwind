...
import { FintwindIcon, type FintwindIconName } from '@/components/fintwind-icon'
...
export function ControlMenu({
  ...
  return (
    ...
    >
      ...
      >
        {icon && <FintwindIcon className="size-[11px] text-[var(--text-tertiary)]" name={icon} />}
        {children ?? <span className="truncate">{label}</span>}
        {caret && <FintwindIcon className="size-2.5 text-[var(--text-ghost)]" name="chevronDown" />}
      </Menu.Trigger>
      <Menu.Portal>
        <Menu.Positioner
        ...
        >
          <Menu.Popup
            aria-label={label}
            className={cn('fintwind-menu-surface', menuClassName)}
            ...
            {items.map((item, index) => (
              <div key={item.id}>
                {item.separatorBefore && index > 0 && (
                  <div className="fintwind-menu-separator" role="separator" />
                )}
                ...
                >
                  {item.icon && <FintwindIcon className="size-3 text-current" name={item.icon} />}
                  ...
                  {item.selected && <FintwindIcon className="size-[11px] text-[var(--text-tertiary)]" name="check" />}
                </Menu.Item>
              </div>
            ))}
          </Menu.Popup>
