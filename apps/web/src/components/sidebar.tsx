import type { AgentSession } from '@fintwind/client'
...
import fintwindAppIconUrl from '../../../../website/public/app-icon.png'
      ...
      <ContextMenu.Portal>
        <ContextMenu.Positioner className="z-[100] outline-none">
          ...
          >
            ...
            >
              <FintwindIcon className="size-3" name="pencil" /> {t('common.rename')}
            </ContextMenu.Item>
            <ContextMenu.Separator className="fintwind-menu-separator" />
            <ContextMenu.Item
              className="fintwind-menu-item text-destructive data-[highlighted]:bg-[var(--danger-soft)]"
              onClick={() => {
                restoreMenuFocus.current = false
                setMenuOpen(false)
            ...
            >
              <FintwindIcon className="size-3" name="trash" /> {t('common.remove')}
            </ContextMenu.Item>
          </ContextMenu.Popup>
...
function SessionMetadata({ item, nowSeconds, t }: { item: SessionItem; nowSeconds: number; t: Translator }) {
  ...
  return (
    <span className="flex w-full min-w-0 items-center gap-1.5 text-[11.5px] leading-[15px] text-[var(--text-tertiary)]">
      <FintwindIcon className="size-[11px] shrink-0" name="folder" />
...
function SessionStatus({ status, t }: { status: AgentSession['status']; t: Translator }) {
  ...
  if (status === 'working' || status === 'connecting') {
    return <FintwindIcon label={t('sidebar.status_working')} className="size-3 text-[var(--success)] motion-safe:animate-spin" name="loaderCircle" />
  }
  if (status === 'waiting') {
    return <FintwindIcon label={t('sidebar.status_waiting')} className="size-3 text-[var(--warning)]" name="alert" />
  }
  return <FintwindIcon label={t('sidebar.status_failed')} className="size-3 text-destructive" name="x" />
}
