...
describe('selected task removal', () => {
  test('opens the newest remaining task in the same project', () => {
    const project = { ...createProject('/repos/fintwind'), id: 'fintwind' }
  ...
  test('creates a fresh projectless workspace after deleting its last task', () => {
    const project = {
      ...createProject('/home/user/.fintwind/projects/old-task'),
      id: 'projectless',
      name: 'No project',
    }
    const removed = createSession(project.id, 'codex', false)
...
function memoryStorage(initial?: string) {
  ...
  if (initial !== undefined) values.set('fintwind.navigation', initial)
  return {
