    ...
    fn migrate_loaded(&mut self) {
        for session in &mut self.sessions {
            let checkpoint_totals_current = session.turns.iter().all(|turn| {
                turn.checkpoint
                    .as_ref()
                    .is_none_or(fintwind_protocol::model::Checkpoint::totals_are_current)
            });
            let before = (
                session.turns.len(),
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
            ...
            .map_err(to_io_error)?
        {
            ResponsePayload::TaskState {
                ...
                fintwind_protocol::projectless::set_workspace_root(projectless_root);
...
pub fn read_remote_reference(
    ...
    let command = if fintwind_protocol::blob::is_reference(reference) {
        Command::ReadBlob {
            reference: reference.to_owned(),
        }
    } else if reference.starts_with(fintwind_protocol::attachments::ATTACHMENT_SCHEME) {
