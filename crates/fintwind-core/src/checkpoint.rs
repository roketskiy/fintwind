...
const TURN_START_METADATA_PREFIX: &str = "Fintwind-Turn-Start: ";
...
pub fn checkpoint_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/fintwind/session-{session_id}-turn-{turn_count}")
}

pub fn turn_start_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/fintwind/session-{session_id}-turn-start-{turn_count}")
}

pub fn turn_diff_base_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/fintwind/session-{session_id}-turn-diff-{turn_count}")
}
...
fn capture_worktree_commit_from(
    ...
    let temporary_index = common_dir.join(format!("fintwind-checkpoint-index-{}", Uuid::new_v4()));
...
fn session_checkpoint_ref_commits(cwd: &Path, session_id: Uuid) -> SessionCheckpointRefs {
    let prefix = format!("refs/fintwind/session-{session_id}-");
    git_output(
        cwd,
        [
            "for-each-ref",
    ...
    S: AsRef<OsStr>,
{
    ...
    if identity {
        command
            .env("GIT_AUTHOR_NAME", "Fintwind")
            .env("GIT_AUTHOR_EMAIL", "fintwind@localhost")
            .env("GIT_COMMITTER_NAME", "Fintwind")
            .env("GIT_COMMITTER_EMAIL", "fintwind@localhost");
    }
