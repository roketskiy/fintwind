...
#[cfg(unix)]
const SHELL_ENV_COMMAND: &str = "/usr/bin/env -0 > \"$FINTWIND_SHELL_ENV_CAPTURE_FILE\"";
...
/// Build a command with the environment a terminal-launched Fintwind normally
...
///
/// Fintwind's Windows build is a GUI-subsystem binary with no console of its own,
...
/// Normalize a Fintwind-owned provider thread before a dependency spawns the child
...
/// desktop-launched Fintwind already has the `PATH` a terminal would — unlike
/// LaunchServices and desktop-file launches, which is what the probe exists
/// for. Children inherit Fintwind's own environment.
#[cfg(not(unix))]
pub fn refresh_from_default_shell() -> bool {
    true
}
...
pub fn default_terminal_shell() -> PathBuf {
    ...
    // see the note in fintwind-client's `unix_terminal_shell_candidates`. The
...
fn capture_shell_environment(
    ...
    command
        ...
        .env("FINTWIND_SHELL_ENV_CAPTURE_FILE", capture.path())
...
fn is_shell_capture_variable(name: &OsStr) -> bool {
    [
        "FINTWIND_SHELL_ENV_CAPTURE_FILE",
        "DISABLE_AUTO_UPDATE",
...
impl ShellEnvironmentCapture {
    fn create() -> Option<Self> {
        for _ in 0..16 {
            ...
            let path =
                std::env::temp_dir().join(format!(".fintwind-shell-env-{}-{id}", std::process::id()));
...
mod tests {
    ...
    fn spawn_unblocks_sigchld_in_the_child_and_restores_the_caller() {
        if std::env::var_os("FINTWIND_SIGCHLD_CHILD_PROBE").is_some() {
        ...
        command
            ...
            .env("FINTWIND_SIGCHLD_CHILD_PROBE", "1");
    ...
    fn a_bare_name_resolves_through_pathext() {
        let directory = std::env::temp_dir().join(format!("fintwind-pathext-{}", std::process::id()));
    ...
    fn parses_null_delimited_environment_without_losing_value_contents() {
        let environment = parse_shell_environment(
            b"PATH=/Users/example/.fnm/current/bin:/usr/bin\0TOKEN=line one\nline two=rest\0EMPTY=\0FINTWIND_SHELL_ENV_CAPTURE_FILE=/tmp/capture\0",
        )
    ...
    fn captures_environment_from_a_shell_process() {
        let id = SHELL_ENV_CAPTURE_ID.fetch_add(1, Ordering::Relaxed);
        let directory =
            std::env::temp_dir().join(format!("fintwind-command-env-test-{}-{id}", std::process::id()));
        fs::create_dir(&directory).expect("create shell fixture directory");
        let shell = directory.join("fake-shell");
        fs::write(
            &shell,
            "#!/bin/sh\n/usr/bin/printf 'PATH=/Users/example/.fnm/current/bin:/usr/bin\\000FINTWIND_TEST_TOKEN=from-shell\\000' > \"$FINTWIND_SHELL_ENV_CAPTURE_FILE\"\n",
        )
        ...
        assert_eq!(
            ...
            vec![
                ...
                (
                    OsString::from("FINTWIND_TEST_TOKEN"),
