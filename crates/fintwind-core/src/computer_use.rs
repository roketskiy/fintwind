...
pub use fintwind_protocol::computer_use::{
...
fn helper_app_path() -> anyhow::Result<PathBuf> {
    ...
    let contents = macos
        .parent()
        .ok_or_else(|| anyhow!("fintwind app bundle is malformed"))?;
    let app_name = executable
        .file_name()
        .and_then(|name| name.to_str())
        .ok_or_else(|| anyhow!("fintwind executable name is invalid"))?;
    let helper_name = format!("{app_name} Computer Use");
    let path = contents.join("Helpers").join(format!("{helper_name}.app"));
    if !path.is_dir() {
        bail!("Computer Use helper is missing from this fintwind build")
    }
    Ok(path)
}
...
pub fn helper_display_name() -> String {
    host_executable_path()
        ...
        .unwrap_or_else(|| "fintwind Computer Use".into())
}
...
pub fn js_repl_server_path() -> anyhow::Result<PathBuf> {
    ...
    let contents = macos
        .parent()
        .ok_or_else(|| anyhow!("fintwind app bundle is malformed"))?;
    let path = contents.join("Resources").join("fintwind_js_repl");
    if !path.is_file() {
        bail!("fintwind JavaScript REPL is missing from this fintwind build")
    }
    Ok(path)
}
...
pub fn pi_extension_path() -> anyhow::Result<PathBuf> {
    ...
    if !path.is_file() {
        bail!("fintwind Pi Computer Use extension is missing from this fintwind build")
    }
    Ok(path)
}
...
/// responsible application. A helper launched from inside Fintwind's bundle is
/// therefore attributed to Fintwind even though the capture API runs in the
...
fn install_helper_app(source: &Path) -> anyhow::Result<PathBuf> {
    ...
    let install_root = application_support.join("Fintwind").join("Computer Use");
...
fn helper_install_matches(source: &Path, destination: &Path) -> anyhow::Result<bool> {
    ...
    let fingerprint = Path::new("Contents/Resources/.fintwind-helper-fingerprint");
...
pub fn skill_root_path() -> anyhow::Result<PathBuf> {
    ...
    if !path.join("fintwind-computer-use").join("SKILL.md").is_file() {
        bail!("fintwind Computer Use skill is missing from this fintwind build")
    }
    Ok(path)
}
...
fn host_executable_path() -> anyhow::Result<PathBuf> {
    std::env::var_os(crate::APP_EXECUTABLE_ENV)
        ...
        .map(PathBuf::from)
        .map(Ok)
        .unwrap_or_else(|| std::env::current_exe().context("fintwind executable path is unavailable"))
}
