...
fn build_opencode_computer_use_config(
    ...
    mcp.insert(
        "fintwind_js_repl".into(),
        serde_json::json!({
            "type": "local",
            "command": [repl_path.display().to_string()],
            "enabled": true,
            "environment": {
                "FINTWIND_COMPUTER_USE_SERVER": server_path.display().to_string(),
                "FINTWIND_COMPUTER_USE_PROCESS_DIRECTORY": process_directory.display().to_string(),
            },
        }),
    );
...
pub(super) fn opencode_computer_use_environment(
    ...
    vec![
        ...
        (
            "FINTWIND_COMPUTER_USE_SERVER".to_owned(),
            config.base.server_path.display().to_string(),
        ),
        (
            "FINTWIND_COMPUTER_USE_PROCESS_DIRECTORY".to_owned(),
...
mod tests {
    ...
    fn opencode_computer_use_config_preserves_existing_inline_config() {
        let content = build_opencode_computer_use_config(
            ...
            Path::new("/Applications/Fintwind Computer Use"),
            Path::new("/Applications/Fintwind.app/Contents/Resources/fintwind_js_repl"),
            Path::new(
                "/Applications/Fintwind.app/Contents/Resources/skills/fintwind-computer-use/SKILL.md",
            ),
            Path::new("/tmp/fintwind computer use/session"),
        )
        .unwrap();
        ...
        assert_eq!(
            value
                .pointer("/mcp/fintwind_js_repl/command/0")
                .and_then(Value::as_str),
            Some("/Applications/Fintwind.app/Contents/Resources/fintwind_js_repl")
        );
        assert_eq!(
            value
                .pointer("/mcp/fintwind_js_repl/environment/FINTWIND_COMPUTER_USE_SERVER")
                .and_then(Value::as_str),
            Some("/Applications/Fintwind Computer Use")
        );
        assert_eq!(
        ...
        assert!(value.pointer("/mcp/fintwind_computer_use").is_none());
    }
}
