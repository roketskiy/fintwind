...
pub use fintwind_protocol::git::{AgentInvocation, CommitSnapshot as Snapshot};
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("fintwind-commit-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        run_git(&root, &["init", "-b", "main"]);
        run_git(&root, &["config", "user.name", "Fintwind Tests"]);
        run_git(&root, &["config", "user.email", "fintwind@example.com"]);
