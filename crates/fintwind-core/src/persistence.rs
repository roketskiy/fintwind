...
//! `~/.fintwind/app.json` for Release, daemon preferences in
//! `~/.fintwind/settings.json`, and binary payloads in [`crate::blob_store`].
//! Of the configuration documents, only the desktop file is written here;
//! daemon settings cross the RPC boundary and are persisted by `fintwind-daemon`.
//!
...
/// at `~/.fintwind/app.json` without exposing app-managed state or daemon-owned
/// provider policy.
...
pub struct StateStore {
    ...
    /// Release uses the explicit cross-client Fintwind configuration directory.
...
impl StateStore {
    ...
    /// Local database owner used inside `fintwind-daemon`. It never reads or
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
        ...
        let app_state_is_saved = if !self.desktop_files {
            ...
            // Seed the state file before the first save so a crash between
            // database writes and JSON writes cannot leave it absent.
...
mod tests {
    ...
    fn temporary_directory() -> PathBuf {
        std::env::temp_dir().join(format!("fintwind-state-{}", Uuid::new_v4()))
    }
    ...
    fn missing_settings_and_app_state_are_created_during_load() {
        ...
        store_in(&directory).load().unwrap();
    ...
    fn load_returns_list_columns_and_hydrate_fills_the_transcript() {
        ...
        state.sessions[0].workspace = SessionWorkspace::Worktree {
            ...
            branch: "fintwind/investigate".into(),
        };
    ...
    fn sent_attachment_presentation_round_trips_with_message_rows() {
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("fintwind-blob:abcdef.png".to_owned()),
        };
    ...
    fn default_path_is_build_specific() {
        ...
        {
            assert_eq!(directory, Some(std::ffi::OsStr::new("Fintwind")));
            let configuration_directory = dirs::home_dir()
                .unwrap_or_else(std::env::temp_dir)
                .join(".fintwind");
            assert_eq!(
