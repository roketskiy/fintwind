...
//! random fallback. Fintwind mirrors that layout beneath `~/.fintwind/projects` so
...
fn workspace_root_slot() -> &'static RwLock<Option<PathBuf>> {
    ...
    ROOT.get_or_init(|| {
        RwLock::new(dirs::home_dir().map(|home| home.join(".fintwind").join("projects")))
    })
}

pub fn set_workspace_root(root: Option<PathBuf>) {
    fintwind_protocol::projectless::set_workspace_root(root.clone());
...
/// Existing builds created dated workspaces directly under `~/.fintwind`; keep
...
pub fn create_workspace(prompt: Option<&str>) -> io::Result<Workspace> {
    let root = workspace_root().ok_or_else(|| {
        io::Error::new(
            ...
            "could not locate the home directory for ~/.fintwind/projects",
        )
    })?;
    create_workspace_in(&root, Local::now().date_naive(), None, prompt)
}

/// Move one old dated workspace from `~/.fintwind/<date>/<slug>` into
/// `~/.fintwind/projects/<date>/<slug>` without copying its contents through the
/// client. The oldest layout used `~/.fintwind` itself; that path contains Fintwind's
...
mod tests {
    ...
    fn test_root() -> PathBuf {
        std::env::temp_dir().join(format!("fintwind-projectless-{}", Uuid::new_v4()))
    }
