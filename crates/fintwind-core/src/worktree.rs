...
//! worktree beneath `~/.fintwind/worktrees`, then every workspace consumer uses
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.fintwind/worktrees"))?
        .join(".fintwind/worktrees");
    create_in(
        project_path,
        &root,
        project_id,
...
fn create_in(
    ...
    for index in 0..MAX_CANDIDATES {
        ...
        let branch = format!("fintwind/{name}");
    ...
    let branch = format!("fintwind/{fallback}");
...
mod tests {
    ...
    fn creates_isolated_worktrees_from_the_default_branch() {
        let root = std::env::temp_dir().join(format!("fintwind-worktree-test-{}", Uuid::new_v4()));
        ...
        // the worktree checkout would hand back CRLF where the fixture wrote
        // LF.
        ...
        .unwrap();
        assert_eq!(first.branch, "fintwind/build-a-project-selector");
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(second.branch, "fintwind/build-a-project-selector-2");
