...
/// The language preference Fintwind persists. `System` resolves to one of the
/// locales Fintwind deliberately ships today.
...
mod tests {
    ...
    fn translations_are_complete_and_interpolate_naturally() {
        assert_eq!(
            &*rust_i18n::t!("settings.general", locale = "en"),
            "General"
        );
        assert_eq!(
