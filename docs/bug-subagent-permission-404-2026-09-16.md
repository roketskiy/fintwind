...
父会话派 `code-reviewer` 子代理。子代理对工作区外路径发 `grep` / `glob`（例如 `C:\Users\example\.cargo\git\checkouts`）后，两个工具调用一直停在 `running`，直到用户手动停止。一次现场挂了 **19 分 54 秒**。
  ...
  "permissions": [
    {
      ...
      "resource": "C:/Users/example/.cargo/git/checkouts/*",
      "effect": "allow"
    }
  ]
}
