...
| workspace 定位 | server 进程与 workspace 绑定（以 cwd 为准） | 无 ambient instance：请求经 `x-opencode-directory` header 或 `?directory=` 路由，**缺省回退 `process.cwd()`**（`workspace-routing.ts` `defaultDirectory`）→ **Waku 现有"每 workspace 一个 serve 进程"的 pool 模型依然成立** |

## 3. Waku 现有驱动逐调用点映射

改动主体：`crates/waku-core/src/driver/opencode.rs`、`opencode_session.rs`、`opencode_pool.rs`。

| Waku 现有调用（v1） | v1 兼容面 | v2 协议面（`/api/...`） |
|---|---|---|
...
- 官方 issue [anomalyco/opencode#41217](https://github.com/anomalyco/opencode/issues/41217)：
  ...
  但**当前发布状态未确认**。
- 含义：**Waku 持久化的 `ProviderResumeCursor::OpenCode{session_id}`（v1 `ses_` id）迁移后
...
1. **`crates/waku-core/src/driver/opencode.rs`**（约 1785 行）
...
2. **`crates/waku-core/src/opencode_session.rs`**（约 612 行）
   - fork/消息列表分页响应结构（`{data, cursor}` vs 裸数组）适配。
3. **`crates/waku-core/src/opencode_pool.rs`**（约 365 行）
...
5. 用户输入实际触发 `question.v2.asked` 还是 form（`frm_`），确认与 Waku `UserInputRequested` 的对接方式；
6. v2 `prompt` 不带 `model` 时是否使用会话已设模型（模型切换流程）；
7. 共享 daemon（`opencode2` 默认发现/启动后台服务）与独立 `serve` 并存时，Waku 独立 spawn 的 serve 是否会被 daemon 干扰。
...
1. **鉴权（推翻 §2）**：`OPENCODE_SERVER_PASSWORD` **未设置或空串都强制
   ...
   无鉴权启动在当前发布版不存在。→ Waku 改为**注入随机密码 + 全部请求带
...
5. **权限（推翻 §4 事件名映射）**：权限请求**不进 SSE**，通过
   ...
   "command":"ask"}` 等配置才会产生 `per_` 请求。→ Waku 新增 400ms 权限轮询
   线程，请求形状映射 `action→permission`、`resources→patterns`、`save→always`。
   Waku 的 question 回复仍走 `POST /api/question/{rid}/reply {answers}`。
6. **会话/消息（推翻 §3 部分假设）**：
...
- `crates/waku-core/src/opencode_session.rs`
...
- `crates/waku-core/src/driver/opencode.rs`
...
- `crates/waku-core/src/{model.rs, command_env.rs, opencode_pool.rs, server.rs}`
...
1. **§5 续聊护栏**：Waku 持久化的 v1 `ses_` 会话在 v2 表族中不可读 ——
...
6. **共享 daemon 并存**：用户后台 daemon（`serve --service`）与 Waku 独立
   serve 实测共库共存无干扰（SQLite WAL）；Waku 会话不受 daemon 影响。