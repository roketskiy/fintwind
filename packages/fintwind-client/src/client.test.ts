...
import { FintwindClient, FintwindRpcError, daemonUrl, type WebSocketLike } from "./client";
...
function fixture() {
  ...
  let nextId = 0;
  const client = new FintwindClient({
...
async function connect(client: FintwindClient, sockets: FakeSocket[]): Promise<FakeSocket> {
...
describe("FintwindClient", () => {
  ...
  test("surfaces daemon errors", async () => {
    ...
    await expect(response).rejects.toBeInstanceOf(FintwindRpcError);
  });
  ...
  test("disconnect rejects an in-flight handshake and permits reconnecting", async () => {
    ...
    const firstConnection = client.connect();
    client.disconnect();
    await expect(firstConnection).rejects.toThrow("Fintwind client disconnected");
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("Fintwind daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("Fintwind daemon is disconnected");
  });
...
test("daemonUrl pins the versioned endpoint", () => {
  ...
  expect(daemonUrl("wss://fintwind.example.test")).toBe("wss://fintwind.example.test/v1");
});
