...
export interface FintwindClientOptions {
...
export class FintwindRpcError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "FintwindRpcError";
  }
}
...
/** Browser-safe client for Fintwind's versioned JSON-over-WebSocket protocol. */
export class FintwindClient {
  ...
  constructor(options: FintwindClientOptions) {
  ...
  connect(): Promise<void> {
    ...
    if (this.state === "connecting") {
      return Promise.reject(new Error("Fintwind client is already connecting"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          failHandshake(new Error("Fintwind daemon sent invalid JSON"));
          return;
        }
        ...
        if (!handshakeSettled) {
          ...
          failHandshake(new Error("Fintwind daemon sent an invalid handshake response"));
      ...
      socket.addEventListener("error", () => {
        failHandshake(new Error("Fintwind daemon connection failed"));
      });
      socket.addEventListener("close", () => {
        if (generation !== this.connectionGeneration) return;
        failHandshake(new Error("Fintwind daemon disconnected during handshake"));
        this.markDisconnected(new Error("Fintwind daemon disconnected"));
      });
    });
  }
  ...
  request(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error("timed out waiting for Fintwind daemon"));
  ...
  disconnect(): void {
    this.rejectConnect?.(new Error("Fintwind client disconnected"));
    ++this.connectionGeneration;
    const socket = this.socket;
    this.socket = undefined;
    this.markDisconnected(new Error("Fintwind client disconnected"));
  ...
  private requireSocket(): WebSocketLike {
    if (this.state !== "connected" || !this.socket || this.socket.readyState !== OPEN) {
      throw new Error("Fintwind daemon is disconnected");
    }
    return this.socket;
  }
  ...
  private handleMessage(message: ServerMessage): void {
    if (message.type === "response") {
      ...
      else pending.reject(new FintwindRpcError(message.outcome.error.message));
      return;
    }
