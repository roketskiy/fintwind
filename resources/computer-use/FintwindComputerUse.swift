...
struct FintwindComputerUse {
    ...
    static func serveMCPBridge() async throws {
        ...
        if let processDirectory = ProcessInfo.processInfo.environment["FINTWIND_COMPUTER_USE_PROCESS_DIRECTORY"],
...
private final class UnixListener {
    ...
    init() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("fintwind-computer-use", isDirectory: true)
...
private final class WindowCaptureSession {
    ...
    init(
        ...
        outputQueue = DispatchQueue(label: "sh.fintwind.computer-use.capture.\(window.windowID)")
...
private func post(_ event: CGEvent, to processID: pid_t) {
    ...
    // input to a global event tap. PID-targeted delivery is what lets Fintwind
