...
class WakuMcpClient {
  ...
  private async start(): Promise<void> {
    ...
    child.stderr.on("data", (chunk) => {
      ...
      if (message) console.error(`fintwind JavaScript REPL: ${message}`);
    });
    child.once("error", (error) => this.handleExit(error));
    child.once("exit", (code, signal) => {
      this.handleExit(
        new Error(
          `fintwind JavaScript REPL exited${code === null ? "" : ` with ${code}`}${
            signal ? ` (${signal})` : ""
          }`,
        ),
      );
  ...
  private requestWithoutStart(method: string, params: JsonObject): Promise<unknown> {
    ...
    if (!child) return Promise.reject(new Error("fintwind JavaScript REPL is not running"));
...
function toolResult(result: JsonObject) {
  ...
  if (result.isError === true) {
    const message = content
      ...
      .map((item) => item.text)
      .join("\n");
    throw new Error(message || "fintwind JavaScript execution failed");
  }
