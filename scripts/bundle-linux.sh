...
version="$(cargo metadata --no-deps --format-version 1 | sed -n 's/.*"name":"fintwind","version":"\([^"]*\)".*/\1/p')"
...
install -Dm644 resources/linux/sh.fintwind.desktop \
  "$package_dir/share/applications/sh.fintwind.desktop"
install -Dm644 website/public/app-icon.png \
  "$package_dir/share/icons/hicolor/256x256/apps/sh.fintwind.png"
