    ...
    pub(super) fn schedule_composer_draft_save(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |fintwind, cx| {
            cx.background_executor()
                ...
                .await;
            let Some((store, drafts)) = fintwind
                .update(cx, |fintwind, cx| {
                    if fintwind.composer_draft_save_generation != generation {
                        return None;
                    }
                    fintwind.capture_current_composer_draft(cx);
                    Some((
                        fintwind.composer_draft_store.clone(),
                        fintwind.composer_drafts.clone(),
                    ))
                })
                .ok()
                .flatten()
            ...
            if let Err(error) = result {
                let _ = fintwind.update(cx, |fintwind, cx| {
                    fintwind.show_toast(tr!("errors.save_local_state", error = error));
                    cx.notify();
                });
            }
        })
