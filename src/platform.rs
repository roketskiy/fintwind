...
#[cfg(target_os = "linux")]
pub fn init_reduce_motion(cx: &mut gpui::App) {
    if let Ok(value) = std::env::var("FINTWIND_REDUCE_MOTION")
...
/// delegate (and therefore click responses); Fintwind only supplies content here
...
/// Keep Fintwind's single main window alive when the user closes it. This preserves
