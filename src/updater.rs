...
//! the installer, verify its EdDSA signature, and hand it to Inno Setup in
//! silent mode. The installer closes the running app, replaces it, and starts
//! it again.
//!
...
//! automatic sidebar result; `FINTWIND_FORCE_UPDATER=1` exercises the real
//! download and install from a debug binary.
...
/// The compact state rendered by fintwind. Update details stay off the
/// frame path.
...
/// There is no Sparkle to hand the work to, so this module runs the same
/// contract itself: read a Sparkle-format appcast, compare versions, download
/// the installer, verify its EdDSA signature against the compiled-in
/// public key, and hand it to Inno Setup in silent mode. The
...
/// threads and report back through `UpdaterEvent`, so the sidebar footer and
/// the Settings toggle stay in lockstep with the download.
...
mod windows {
    ...
    /// Read out of `resources/sparkle-public-ed-key.txt` by the build script.
    ...
    impl Updater {
        pub fn init() -> Option<Self> {
            ...
            let forced =
                std::env::var_os("FINTWIND_FORCE_UPDATER").is_some_and(|value| value == "1");
            if cfg!(debug_assertions) && !forced {
                return None;
            }
            if verifying_key().is_none() {
                eprintln!("fintwind updater: compiled-in public key is not a valid ed25519 key");
                return None;
            }
            ...
            // One silent check per launch is the whole schedule.
    ...
    fn download_and_verify(item: &AppcastItem) -> anyhow::Result<PathBuf> {
        let key =
            verifying_key().ok_or_else(|| anyhow::anyhow!("compiled-in public key is unusable"))?;
    ...
    /// Default to checking automatically, and treat an unreadable or absent
    ...
    mod tests {
        ...
        /// The interop the whole update path rests on: releases sign with
        /// Node's Ed25519 in `scripts/appcast-windows.ts`, and this verifies
        /// them. The vector below came from that script with a throwaway key.
        #[test]
