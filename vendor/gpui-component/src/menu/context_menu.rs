...
pub trait ContextMenuExt: ParentElement + Styled {
    ...
    /// Add a context menu with an explicit element ID.
    ///
    /// Use this when rendering multiple context menus under the same parent so
    /// each menu retains independent element state.
