...
const RELEASES_BASE = 'https://releases.fintwind.sh'
...
export const FALLBACK_DOWNLOAD_URL = `${RELEASES_BASE}/fintwind-0.0.1.dmg`
...
export function windowsInstallerUrl(version: string, arch: string) {
  return `${RELEASES_BASE}/fintwind-${version}-${arch}-Setup.exe`
}
...
const fetchLatestRelease = createServerFn({ method: 'GET' }).handler(
  async (): Promise<LatestRelease | null> => {
    try {
      ...
      return {
        version,
        url: `${RELEASES_BASE}/fintwind-${version}.dmg`,
        pubDate,
      }
    } catch {
      return null
